...
@Component({
  selector: "app-third-company-add",
  templateUrl: "./third-company-add.component.html",
  styleUrls: ["./third-company-add.component.scss"],
})
export class ThirdCompanyAddComponent implements OnInit {
  ...
  addThirdCompanyForm = new FormGroup({
  ...
  add() {
    if (this.confirmSubmit() == false) {
      ...
      this.TError = {
        isError: true,
        errorMessage: `This field must contain a valid tax number`,
      };
      this.Error = { isError: true, errorMessage: `` };
    } else {
      this.thirdCompanyService
        .addThirdPartyCompany(this.addThirdCompanyForm.value)
        .subscribe(() => {
  ...
  confirmTaxSubmit(): Boolean {
    var taxInputLength = (<HTMLInputElement>document.getElementById("fax"))
      .value.length;
  ...
  ConfirmTextOpen(){
    Swal.fire({
      title: 'Success!',
      text: "You have successfully added a new third company!",
      icon: 'success',
